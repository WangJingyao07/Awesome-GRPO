...
class Algorithm(AlgorithmBase):
    ...
    def __init__(self, engine, tokenizer, beta: float, clip_param: float, compute_gen_logps: bool, eps_up: float, eps_down: float, **_extra):
        super().__init__(engine, tokenizer, beta=beta, clip_param=clip_param, compute_gen_logps=compute_gen_logps, eps_up=eps_up, eps_down=eps_down)
    ...
    def step(self, batch: Dict) -> torch.Tensor:
        ...
        if 'gen_logps' not in batch:
            raise RuntimeError("DAPO requires 'gen_logps' in batch (old policy log-probs from generation).")
        old_logps = batch['gen_logps'].to(engine.device)        # (B, T)

        # Decoupled Clip
        ratio = torch.exp(per_token_logps - old_logps)          # (B, T)
        lower, upper = 1.0 - self.eps_down, 1.0 + self.eps_up
        ratio_clipped = torch.clamp(ratio, min=lower, max=upper)

        obj = ratio * advantages
        obj_clip = ratio_clipped * advantages
        per_token_loss = -torch.min(obj, obj_clip)              # (B, T)
